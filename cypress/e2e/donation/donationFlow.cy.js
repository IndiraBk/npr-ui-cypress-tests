...
describe('NPR Donation Flow', () => {
  ...
  it('Should switch between Donate to NPR Network and Local Station', () => {
    ...
    cy.contains('label', 'Donate to a local station')
      .should('have.class', 'designation-types__item--selected');
    cy.contains('label', 'Donate to the NPR Network').click();
    cy.contains('label', 'Donate to the NPR Network')
      .should('have.class', 'designation-types__item--selected');
  ...
  it('Should switch between Monthly and One-Time and display correct donation amounts', () => {
    cy.fixture('donationAmounts').then((donationAmounts) => {
      ...
      cy.get('@nprOption').should('have.class', 'designation-types__item--selected');
      cy.get('.donation-recurrence-types').should('be.visible');
  
      cy.contains('.donation-recurrence-types__item', 'Monthly')
        .as('monthlyOption')
        .click();
      cy.get('@monthlyOption').should('have.class', 'donation-recurrence-types__item--selected');
      ...
      cy.contains('.donation-recurrence-types__item', 'One-time')
        .as('oneTimeOption')
        .click();
      cy.get('@oneTimeOption').should('have.class', 'donation-recurrence-types__item--selected');
  ...
  it('Should complete the donation form and payment information section', () => {
    ...
    // Stripe input frames
    cy.get('iframe[title="Secure card number input frame"]', { timeout: 20000 }).should('exist');
